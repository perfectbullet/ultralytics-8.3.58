...
def main(args):
    ...
    for root, dirs, files in os.walk(voc_dir):
        for name in files:
            if name.endswith(('.jpg', '.xml')):
                shutil.move(join(root, name), join(voc_dir, name))
    ...
    # 写入当前的classes.txt
    with open(os.path.join(yolo_dir, 'classes.txt'), 'wt') as f:
        f.write('\n'.join(names))
    ...
    for id in tqdm(image_ids, desc=f'{voc_dir}'):
        ...
        if os.path.exists(old_xml):
            # copy image and covert jpg by opencv
            shutil.copy(old_image, new_image)
            one_img = cv2.imread(old_image)
            cv2.imwrite(new_image, one_img)
